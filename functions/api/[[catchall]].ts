...
app.post('/groups', async (c) => {
  ...
  const row = await c.env.DB.prepare(
    `INSERT INTO groups (name) VALUES (?) RETURNING id, name, created_at as createdAt`
  ).bind(name.trim()).first<Omit<Group, 'teamCount'>>()
...
app.get('/groups/:groupId/teams', async (c) => {
...
app.post('/groups/:groupId/teams', async (c) => {
  const groupId = Number(c.req.param('groupId'))
  const { name } = await c.req.json<{ name: string }>()
  if (!name?.trim()) return c.json({ error: 'name required' }, 400)
  const row = await c.env.DB.prepare(
...
app.post('/games', async (c) => {
  ...
  const gameRow = await c.env.DB.prepare(`
    INSERT INTO games (name, group_id, postpone_as_win, winner_mode, rollover_mode, max_winners, participant_count)
    VALUES (?, ?, ?, ?, ?, ?, ?)
    RETURNING id
  `).bind(
    body.name, body.groupId,
    body.postponeAsWin ? 1 : 0,
    body.winnerMode, body.rolloverMode, body.maxWinners,
    body.playerNames.length
  ).first<{ id: number }>()
...
app.post('/games/:id/participants', async (c) => {
...
app.post('/games/:id/advance', async (c) => {
  const gameId = Number(c.req.param('id'))
  const body = await c.req.json<{
    currentRound: number
    openRoundId: number
  ...
  const stmts = [
    ...body.eliminatedParticipantIds.map(pid =>
      c.env.DB.prepare(
        `UPDATE participants SET is_active = 0, eliminated_in_round = ? WHERE id = ?`
      ).bind(body.currentRound, pid)
    ),
    ...
    body.nextRoundNumber === null
      ? c.env.DB.prepare(`UPDATE games SET status = 'completed', winner_name = ? WHERE id = ?`)
          .bind(body.winnerNames?.join(', ') ?? null, gameId)
      : c.env.DB.prepare(`UPDATE games SET current_round = ? WHERE id = ?`)
          .bind(body.nextRoundNumber, gameId),
    ...(body.nextRoundNumber !== null ? [
      c.env.DB.prepare(`INSERT INTO rounds (game_id, round_number, status) VALUES (?, ?, 'open')`)
        .bind(gameId, body.nextRoundNumber),
    ] : []),
  ]
...
app.post('/games/:id/rollover', async (c) => {
  const gameId = Number(c.req.param('id'))
  const body = await c.req.json<{
    currentRound: number
    openRoundId: number
...
app.get('/sync', async (c) => {
  const [groups, teams, players, games, participants, rounds, picks] = await Promise.all([
    c.env.DB.prepare(`
      SELECT g.id, g.name, g.created_at as createdAt, COUNT(t.id) as teamCount
      FROM groups g LEFT JOIN teams t ON t.group_id = g.id GROUP BY g.id ORDER BY g.created_at
    `).all<Group>(),
